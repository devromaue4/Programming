...
namespace ExitGames.Client.DemoParticle
{
    ...
    public class GameLogic : LoadBalancingClient
    {
        ...
        public GameLogic(string masterServerAddress, string appId, string gameVersion) : base(masterServerAddress, appId, gameVersion)
        {
        ...
        public void Start()
        {
            ...
            //{
            //    this.DebugReturn(DebugLevel.ERROR, "Can't connect to EU server via NameServer: " + this.NameServerAddress);
            //}
            //if (!this.ConnectToNameServer())
            //{
            //    this.DebugReturn(DebugLevel.ERROR, "Can't connect to NameServer: " + this.NameServerAddress);
            //}
            if (!this.Connect(this.MasterServerAddress, this.AppId, this.AppVersion, null, null))
            {
...
//                    this.OpJoinRandomRoom(null, 0);
        ...
        private void SendPositionUpdate()
        {
            if (this.UseInterestGroups)
            {
                ...
                this.loadBalancingPeer.OpRaiseEvent(DemoConstants.EvPosition,this.LocalPlayer.WriteEvMove(), this.SendReliable, new RaiseEventOptions(){InterestGroup = this.LocalPlayer.GetGroup()});
            }
            else
            {
...
//            this.OpCreateRoom(null, new RoomOptions() { customRoomProperties = roomPropsForCreation, customRoomPropertiesForLobby = RoomPropsInLobby }, null);
            this.OpJoinOrCreateRoom("Demo", 0, new RoomOptions() { CustomRoomProperties = roomPropsForCreation, CustomRoomPropertiesForLobby = RoomPropsInLobby });
